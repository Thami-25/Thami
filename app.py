...
def badge_ruptura(rupt):
    ...
    mapa = {
        "C/ Compra":"rupt0", "c/ compra":"rupt0",
        "1 Mês":"rupt1", "2 Meses":"rupt2",
        "3 Meses":"rupt3", "> 6 Meses":"rupt3",
        "4 Meses":"rupt3", "5 Meses":"rupt3", "6 Meses":"rupt3",
    }
    cls = next((v for k,v in mapa.items() if k.lower() in rupt.lower()), "rupt2")
...
def carregar_roteiro():
    try:
        ...
        col_bairro = achar(["address line 4","bairro","address 4"])  or df.columns[3]
        ...
        col_rupt   = achar(["ruptura"])
        col_dev    = achar(["devedor"])
...
def carregar_vendas():
    try:
        ...
        col_cat     = achar(["categoria","category"])
        col_caixas  = achar(["somadecaixas","caixas","qtd"])
        col_data    = achar(["data faturamento","data fat","datafat"])
        ...
        df["_data"]   = pd.to_datetime(df[col_data], errors="coerce") if col_data else None

        df = df[df["_status"] == "VENDA"]

        resumo = df.groupby("_id").apply(lambda x: {
            "impulso": int(x[x["_cat"].str.contains("IMPULSO", na=False)]["_caixas"].sum()),
            "th":      int(x[x["_cat"].str.contains("TAKE HOME|TH", na=False)]["_caixas"].sum()),
            "comprou": len(x) > 0
        }).to_dict()
...
if st.session_state.tela == "selecao":
    vendedores = sorted(df_rot["_vendedor"].dropna().unique().tolist())
    vendedores = [v for v in vendedores if v and v != "nan"]
...
# ── TELA PAINEL ──────────────────────────────────────────────────────────────
    ...
    def render_clientes(df_lista, label):
        ...
        for _, row in df_lista.iterrows():
            ...
            comp_badge = '<span class="bdg compra">Comprou este mês</span>' if comprou else ""
...
<div class="{card_cls}">
  ...
  <div class="bdgs">{badge_ruptura(rupt)}{dev_badge}{comp_badge}</div>
  <div class="srow">
    ...
    with aba_semana:
        ...
        for dia in dias_ordem:
            cl = dfv[(dfv["_dia"]==dia) & dfv["_freq"].apply(lambda f: visita_hoje(f, semana_hoje))]
            if not cl.empty:
